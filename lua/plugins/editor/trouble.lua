...
vim.keymap.set(
	...
	{ desc = "Toggle all project diagnostics errors warnings" }
)
vim.keymap.set(
	"n",
	"<leader>db",
	"<cmd>Trouble diagnostics toggle filter.buf=0<cr>",
	{ desc = "Toggle diagnostics for current buffer only" }
)
vim.keymap.set(
	"n",
	"<leader>ds",
	"<cmd>Trouble symbols toggle focus=false<cr>",
	{ desc = "Toggle symbols outline sidebar (functions classes)" }
)
